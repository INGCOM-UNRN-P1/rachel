---
title: "Guía de Extensión y Creación de Plugins: rachel"
subtitle: "Manual de integración, desarrollo de extensiones y uso de la API Python de rachel"
author: "Cátedra de Algoritmos y Programación"
date: "2026-08-31"
---

(manual-rachel-plugins)=
# Guía de Extensión y Plugins: rachel

````{abstract}
Esta guía técnica detalla cómo desarrollar extensiones, crear nuevos plugins e integrar programáticamente **`rachel`** en herramientas de evaluación, entornos de integración continua (CI/CD) o scripts docentes.
````

---

(manual-rachel-plugins-arquitectura)=
## 1. Arquitectura de Extensión

`rachel` provee una arquitectura modular desacoplada basada en puntos de entrada (Entry Points) estándar de Python (`[project.entry-points]`) o interfaces de inyección funcional:

- **Mecanismo de Extensión Principal**: `Extensiones y Reglas Personalizadas para rachel`.
- **Punto de Entrada Oficial**: `rachel.plugins`.
- **Formato de Comunicación**: Estructuras de datos serializables JSON / Pydantic models.

---

(manual-rachel-plugins-tutorial)=
...
class CustomPlugin:
    """Plugin de extensión para rachel."""
    name = "custom_rule"
...
Para que `rachel` descubra y cargue automáticamente tu plugin, agregalo en tu `pyproject.toml`:

````{code-block} toml
[project.entry-points."rachel.plugins"]
mi_plugin = "mi_paquete.modulo:MiPlugin"
````

### Paso 3: Instalar y Verificar el Plugin

Instalá tu extensión en modo editable y comprobá que `rachel` la reconozca:
...
(manual-rachel-plugins-sdk)=
## 3. Conexión Programática mediante la API Python

Podés importar y ejecutar `rachel` directamente desde scripts de Python sin invocar subprocesos:
...
resultado = rachel.ejecutar_analisis(
...
(manual-rachel-plugins-ci)=
## 4. Integración en Pipelines de CI/CD (GitHub Actions / GitLab CI)

Podés integrar `rachel` en tus flujos automatizados de Git para bloquear entregas que no cumplan los requisitos de cátedra:
...
jobs:
  auditoria:
    ...
    steps:
      ...
      - name: Instalar rachel
        run: pip install -e ./rachel
        
      - name: Ejecutar Auditoría
        run: rachel check src/ include/ --json > reporte.json
````

---

(manual-rachel-plugins-ejercicios)=
